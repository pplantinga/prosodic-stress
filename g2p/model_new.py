...
class Seq2SeqModel():
    ...
    def __init__(self, encoder_cell, decoder_cell, vocab_size, embedding_size, layers, keep_prob,
                 bidirectional=True,
                 attention=False,
                 debug=False):
        self.debug = debug
    ...
    def _init_bidirectional_encoder(self):
        with tf.variable_scope("BidirectionalEncoder") as scope:
            ...
            if isinstance(encoder_fw_state, LSTMStateTuple):
                ...
                self.encoder_state = tuple(LSTMStateTuple(
                    c=tf.concat((encoder_fw_state[i].c, encoder_bw_state[i].c), 1),
                    h=tf.concat((encoder_fw_state[i].h, encoder_bw_state[i].h), 1)) for i in range(self.layers))
    ...
    def _init_decoder(self):
        with tf.variable_scope("Decoder") as scope:
            ...
            if not self.attention:
                ...
                if self.layers > 1:
                    #c = tf.zeros_like(self.encoder_state[0].c)
                    #h = tf.zeros_like(self.encoder_state[0].h)
                    #encoder_state = tuple([self.encoder_state[-1]] + [LSTMStateTuple(c=c, h=h)] * (self.layers - 1))
                    encoder_state = tuple([self.encoder_state[-1]] * self.layers)
                else:
                    encoder_state = self.encoder_state
    ...
    def make_train_inputs(self, input_seq, target_seq, learning_rate, keep_prob):
    ...
    def make_inference_inputs(self, input_seq, keep_prob):
        inputs_, inputs_length_ = helpers.batch(input_seq)
        return {
            self.encoder_inputs: inputs_,
            self.encoder_inputs_length: inputs_length_,
